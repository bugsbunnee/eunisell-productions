...
const Process: React.FC = () => {
  return (
    <section className="bg-white flex flex-col gap-16 lg:gap-20 items-center px-9 md:px-16 lg:px-36 py-16 lg:py-35">
      ...
      <div className="relative flex flex-col sm:flex-row items-start justify-center max-w-300 w-full">
        <div className="absolute hidden sm:block left-[15%] right-[15%] top-8 h-0.5 bg-line" />
        <div className="absolute hidden sm:block left-[15%] right-1/2 top-8 h-0.5 bg-secondary" />

        {steps.map(({ number, title, description }, index) => (
          <div key={number} className="flex flex-1 flex-col items-center gap-6 relative">
            <div
