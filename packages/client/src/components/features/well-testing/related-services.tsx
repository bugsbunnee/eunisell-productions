...
const RelatedServices: React.FC = () => {
  ...
  return (
    <section className="bg-ice-blue flex flex-col items-center px-9 md:px-16 lg:px-36 py-16 lg:py-32">
      <FadeIn className="flex flex-col gap-20 w-full max-w-360">
        ...
        <Stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7 w-full">
          {relatedServices.map(({ number, title, description, image, imageBg }) => (
            <StaggerItem key={number} hover className="w-full">
              <button
                type="button"
                onClick={() => navigate(paths.services)}
                className="group bg-white border border-line rounded-2xl shadow-sm overflow-hidden flex flex-col items-start text-left w-full"
              >
