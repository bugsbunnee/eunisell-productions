    ...
    <section className="bg-surface flex flex-col gap-16 lg:gap-30 items-start px-9 md:px-16 lg:px-36 py-16 lg:py-35">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 w-full">
        {stats.map(({ value, title, description }) => (
          <div key={title} className="flex flex-col justify-between gap-8 h-75 items-start bg-white border border-line rounded-2xl p-10 shadow-[0px_1px_1px_rgba(0,0,0,0.05)]">
      ...
      <div className="flex flex-col gap-12 items-center w-full">
        ...
        <div className="flex flex-wrap gap-4 items-center justify-center max-w-288">
          {serviceLines.map((line) => (
            <span key={line} className="bg-accent rounded-full px-8 py-4 font-light text-white text-[15px] whitespace-nowrap">
              {line}
            </span>
          ))}
        </div>
