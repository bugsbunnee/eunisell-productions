...
const Ecosystem: React.FC = () => {
  return (
    <section className="bg-white flex flex-col gap-24 items-center px-9 md:px-16 lg:px-36 py-16 lg:py-32">
      <FadeIn className="flex flex-col gap-4 items-center max-w-168 w-full">
      ...
      <Stagger className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-12 items-center justify-items-center w-full max-w-160 lg:hidden">
        ...
        <motion.div
          ...
          className="bg-secondary border-8 border-white rounded-full drop-shadow-[0px_0px_40px_rgba(0,131,206,0.4)] flex items-center justify-center shrink-0 size-32 col-span-2 sm:col-span-3"
        >
          <p className="font-light text-white text-xl tracking-[2.4px]">HSEQ</p>
        </motion.div>
      </Stagger>

      <Stagger amount={0.4} className="hidden lg:flex relative items-center justify-center h-150 w-full max-w-312">
        ...
        {nodes.map(({ icon, label, position }) => (
          <StaggerItem key={label} className={`absolute ${position}`}>
            <Node icon={icon} label={label} />
          </StaggerItem>
        ))}
        ...
        <motion.div
          ...
          viewport={{ once: true, amount: 0.5 }}
          transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1], delay: 0.3 }}
          className="bg-secondary border-12 border-white rounded-full drop-shadow-[0px_0px_40px_rgba(0,131,206,0.4)] flex items-center justify-center shrink-0 size-50"
        >
