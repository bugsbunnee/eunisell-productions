...
const Pillars: React.FC = () => {
  return (
    <section className="bg-ice-blue flex flex-col gap-10 items-center px-9 md:px-16 lg:px-36 py-16 lg:py-32">
      ...
      <Stagger className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full max-w-360">
        {pillars.map(({ icon, title, description }) => (
          <StaggerItem
            key={title}
            hover
            className="bg-white border-t-4 border-secondary rounded-2xl drop-shadow-[0px_1px_1px_rgba(0,0,0,0.05)] flex flex-col gap-4 items-start p-10 h-full"
          >
